...
> 본 프로젝트는 대학생 수준에서 직접 영상/이미지를 촬영하지 않고도 공개 데이터셋을 활용해 영상처리, 딥러닝, 이상탐지, 시각화를 모두 경험하는 것을 목표로 합니다.
...
산업 이상탐지는 실제 제조업에서 활용되는 주제이지만, 공개 데이터셋과 오픈소스 라이브러리를 활용하면 대학생 수준에서도 충분히 구현할 수 있다.
